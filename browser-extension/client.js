...
async function initialize() {
    ...
    // Inject Continuous Memory Harvester (using manifest v3 compliant script injection to bypass CSP)
    const harvester_script = document.createElement('script');
    harvester_script.src = chrome.runtime.getURL('harvester.js');
    harvester_script.onload = function() {
        this.remove();
    };
...
async function extract_memory_pins() {
    return new Promise((resolve) => {
        ...
        // Send a request to the Harvester running in the MAIN world
        window.postMessage({ type: 'PINTEREST_REQUEST_DATA' }, '*');
...
async function fetch_pin_media(pin_slug) {
    ...
    try {
        ...
        if (!pinData) {
            ...
            // LAST RESORT: If the harvester had cached data (even fallback|| URLs),
            // use it instead of giving up and falling back to thumbnail
            if (memory_cached_pins[pin_id] && memory_cached_pins[pin_id].length > 0) {
